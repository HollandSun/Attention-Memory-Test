...
p.NumTrials = 10;   % TEST: 原为60，测试版缩减为10

% ---- 时间参数 ----
% TEST: DistInterval全部用1秒，避免等待太久
% 原为Shuffle([ones(1,20) ones(1,20)*3 ones(1,20)*5])
p.DistInterval = ones(1, p.NumTrials);
p.FrameRate    = 4;
...
% TEST: 10个trial直接shuffle，原为按DistInterval三组分别shuffle
p.ShiftCues = zeros(1, p.NumTrials); % 2=shift, 1=hold
if p.block_order(run) == 'S'
    p.ShiftCues = Shuffle([ones(1,8)*2 ones(1,2)]);   % 8 shift + 2 hold
else   % 'H'
    p.ShiftCues = Shuffle([ones(1,2)*2 ones(1,8)]);   % 2 shift + 8 hold
end
...
block_imgs = block_imgs(1:p.NumTrials);   % TEST: 只取前10张，原为全部60张
...
p.StartingLocations(2:p.NumTrials) = imageLocation(1:p.NumTrials-1);  % TEST: 原为硬编码2:60

p.Response    = Shuffle([ones(1,p.NumTrials/2) ones(1,p.NumTrials/2)*2]);  % 5 odd + 5 even
...
fprintf('Accuracy: %d\n', acc);
WaitSecs(4);   % TEST: 原为29.5秒，测试版缩短为4秒
Screen('Flip', window, p.prevFlip + 5);   % TEST: 原为p.prevFlip + 30
while KbCheck; end
